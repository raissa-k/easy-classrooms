...
module.exports = {
	...
	getClassroom: async (req, res) => {
		try {
			...
			let foundLessons
			if (enrollment) {
				const lessons = enrollment.lessonCompletion.flat()
				foundLessons = lessons
			}
			res.render('classroom.ejs', { classrooms: publicClassroom, lessons: publicClassroom.lessons, enrollment, enrolledLessons: foundLessons, user: req.user, isTeacher })
		} catch (err) {
			req.flash('error', {
	...
	getTeacherDashboard: async (req, res) => {
		if (!req.user.educator) {
			...
			try {
				const userClassrooms = await Classroom.find({ creator: req.user }).populate('lessons')
	...
	getClassroomManagement: async (req, res) => {
		try {
			const managedClassroom = await Classroom.findOne({
				accessName: req.params.accessName,
				creator: req.user
			}).populate('lessons')
	...
	createClassroom: async (req, res, next) => {
		const newAccessName = randomAccess
	...
	editClassroom: async (req, res) => {
		...
		if (!req.file) {
			...
			const currendCloudId = foundClassroom.cloudinaryId
			try {
				const result = await cloudinary.uploader.upload(imageToUpload,
					{ width: 400, height: 300, crop: 'fill', gravity: 'auto' })
	...
	deleteClassroom: async (req, res) => {
		...
		const classroomToDelete = await Classroom.findById(classroomId)
		const lessonsToDelete = classroomToDelete.lessons.map((obj) => obj._id)
